...
class PartImage
{
    ...
    public static function kindFromCategory(?string $categoryTitle): ?string
    {
        ...
        $map = [
            ...
            // --- Склоочисник: 'щітк'/'склоочисн' мусять іти ПЕРЕД 'скло',
            //     інакше «Щітки склоочисника» отримували фото дзеркала. ---
            'склоочисн' => 'wiper',
            'щітк' => 'wiper',
            'двірник' => 'wiper',
            ...
            // --- Загальні, наприкінці: щоб не перехоплювали конкретніші
            //     правила вище («гальмівн рідин» → oil, «антифр» → coolant). ---
            'рідин' => 'oil',
            'комплект' => 'filter',
            // «Деталі для ТО» — теж сервісні запчастини. Обидва слова обовʼязкові,
            // щоб правило не хапало «Деталі салону» (там килимок) чи будь-що з «то».
            'детал то' => 'filter',
        ...
        foreach ($map as $needle => $kind) {
            // Багатослівні правила («гальмівн рідин») як підрядок НЕ спрацьовували:
            // між словами стоїть українське закінчення («гальмівнА рідинА»). Тому
            // для них вимагаємо наявність кожного слова окремо.
            if (mb_strpos($needle, ' ') !== false) {
                $all = true;
                foreach (explode(' ', $needle) as $word) {
                    if ($word !== '' && mb_strpos($t, $word) === false) {
                        $all = false;
                        break;
                    }
                }
