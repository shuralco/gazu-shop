...
class SpecialsComponent extends Component
{
    ...
    {
        $this->sortList = [
            'default' => ['title' => __('general.sort_popularity'), 'order_field' => 'id', 'order_direction' => 'desc'],
            ...
            'discount-desc' => ['title' => __('general.sort_discount'), 'order_field' => 'old_price', 'order_direction' => 'desc'],
        ];
    }
    ...
    public function updatedPage($page)
    {
        ...
        $this->dispatch('page-updated', title: shopName()." :: Акції{$page_title}");
    }
    ...
    {
        // Get price range for special products
        ...
        // Get categories that have special products
        $categories = Category::whereHas('products', function ($query) {
            $query->where(function ($subQuery) {
                $subQuery->where('old_price', '>', 0)
                    ->orWhere('is_hit', 1)
                    ->orWhere('is_new', 1);
            });
        })->get();
        ...
        // Build products query for special items
        $products = Product::query()
            ->where(function ($query) {
                $query->where('old_price', '>', 0)  // Products with old price (discounted)
                    ->orWhere('is_hit', 1)        // Hit products
                    ->orWhere('is_new', 1);       // New products
            })
        ...
        // Apply sorting
        if ($this->sort === 'discount-desc') {
            $products->orderByRaw('(old_price - price) DESC');
        } else {
            $products->orderBy($this->sortList[$this->sort]['order_field'], $this->sortList[$this->sort]['order_direction']);
        }
        ...
        $title = 'Акції'.($page > 1 ? " :: Сторінка - {$page}" : '');

        return view('livewire.product.specials-component', [
            'products' => $products,
