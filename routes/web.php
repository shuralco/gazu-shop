...
// ТИМЧАСОВО: db-tool (розміри + безпечна чистка логів/кешу/черг/орфанів). Видалити після.
Route::get('/__dbtool', function (\Illuminate\Http\Request $request) {
    $u = auth()->user();
    ...
    if ($request->query('action') === 'sizes') {
        $rows = $DB->select(
            ...
            .'FROM information_schema.tables WHERE table_schema = ? ORDER BY (data_length+index_length) DESC LIMIT 30', [$db]
        );
        return response()->json(['tables' => $rows]);
    }

    if ($request->query('action') === 'clean') {
        // Безпечно очищувані: логи/телеметрія/черги/сесії/кеш/transient.
        // НЕ чіпаємо: products/categories/orders/users/brands/filters/settings/np_*/up_*.
        $safe = [
            'activity_log', 'telescope_entries', 'telescope_entries_tags', 'telescope_monitoring',
            'pulse_entries', 'pulse_aggregates', 'pulse_values',
            'jobs', 'job_batches', 'failed_jobs',
            'sessions', 'cache', 'cache_locks',
            'shipping_api_logs', 'password_reset_tokens', 'personal_access_tokens',
            'audits', 'audit_logs', 'http_logs', 'request_logs', 'webhook_calls',
        ];
        $out = [];
        ...
        try {
            foreach ($safe as $tbl) {
                if (\Schema::hasTable($tbl)) {
                    try { $out[$tbl] = $DB->table($tbl)->delete(); } catch (\Throwable $e) { $out[$tbl] = 'ERR'; }
                }
            }
            if (\Schema::hasTable('category_filters')) {
                $catIds = $DB->table('categories')->pluck('id')->all();
                $out['category_filters_orphans'] = $DB->table('category_filters')->whereNotIn('category_id', $catIds ?: [0])->delete();
            }
            $prodIds = $DB->table('products')->pluck('id')->all();
            if (\Schema::hasTable('inventory')) {
                $out['inventory_orphans'] = $DB->table('inventory')->whereNotIn('product_id', $prodIds ?: [0])->delete();
            }
            if (\Schema::hasTable('seo_meta')) {
                $out['seo_meta_product_orphans'] = $DB->table('seo_meta')
                    ->where('seoable_type', 'like', '%Product%')->whereNotIn('seoable_id', $prodIds ?: [0])->delete();
            }
        } finally {
    ...
    return response()->json(['hint' => 'sizes | clean']);
})->middleware(['web', 'auth']);
