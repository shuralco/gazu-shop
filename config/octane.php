...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | package_max_length — стеля розміру ОДНОГО запиту в самому Swoole. Дефолт
    | ~2 MB, і він відсікав би аплоад фото навіть після підняття лімітів nginx
    | і PHP: nginx би пропустив, а Swoole обірвав з'єднання, і в адмінці знову
    | було б безадресне «failed to upload». Тримаємо ліміт узгодженим з
    | client_max_body_size (docker/nginx.conf) і post_max_size (php-ini).
    |
    */
