@props(['productId', 'variant' => 'card'])
{{-- Out-of-stock notification: button + inline popover (як callback-popover).
     variant=card → text button «Сповістити коли з'явиться»
     variant=full → full-width primary button --}}
<div x-data="{
        open: false,
        email: '',
        phone: '',
        ...
        async submit() {
            ...
            this.busy = true;
            try {
                const r = await fetch('{{ route('gazu.stock.notify') }}', {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': window.GAZU_CSRF, 'Accept': 'application/json', 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: new URLSearchParams({ product_id: '{{ $productId }}', email: this.email, phone: this.phone })
                });
    ...
    @if($variant === 'full')
        <button type="button" @click="toggle()"
                class="w-full py-2.5 bg-[var(--gazu-ink)] hover:bg-[var(--gazu-ink-2)] text-white border-0 rounded-md text-[13px] font-medium cursor-pointer inline-flex items-center justify-center gap-1.5 transition-colors">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
            Сповістити коли з'явиться
        </button>
    @else
        <button type="button" @click="toggle()"
                class="flex-1 min-w-0 py-2.5 bg-white text-[var(--gazu-ink)] border border-[var(--gazu-line)] hover:border-[var(--gazu-ink)] rounded-md text-[13px] font-medium cursor-pointer inline-flex items-center justify-center gap-1.5 whitespace-nowrap transition-colors">
    ...
    <div x-show="open" x-cloak
         ...
         class="absolute z-50 bottom-full left-0 right-0 mb-2 bg-white border border-[var(--gazu-line)] rounded-xl shadow-[0_12px_32px_-8px_rgba(14,27,44,0.20)] overflow-hidden">
        ...
        <form x-show="!sent" @submit.prevent="submit()" class="px-3.5 pb-3.5 flex flex-col gap-2">
            ...
            <button type="submit" :disabled="busy"
                    class="w-full py-2 bg-[var(--gazu-blue)] text-white border-0 rounded text-[13px] font-semibold transition-colors inline-flex items-center justify-center gap-1.5 disabled:opacity-70 disabled:cursor-wait hover:bg-[var(--gazu-blue-600)]">
