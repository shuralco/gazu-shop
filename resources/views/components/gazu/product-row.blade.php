...
<div class="bg-[var(--gazu-surface)] border border-[var(--gazu-line)] rounded-lg p-3 flex gap-3 hover:border-[var(--gazu-line-2)] transition-colors">
    ...
    <div class="shrink-0 flex flex-col items-end gap-2 min-w-[160px]">
        ...
        @else
            <a wire:navigate href="{{ $url }}" class="w-full px-4 py-2 bg-[var(--gazu-ink)] text-[var(--gazu-on-brand)] border-0 rounded-md text-[13px] font-medium cursor-pointer inline-flex items-center justify-center gap-1.5 whitespace-nowrap hover:bg-[var(--gazu-ink-2)] no-underline">Деталі</a>
        @endif
    </div>
</div>
