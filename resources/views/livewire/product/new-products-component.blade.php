<div>
    @section('metatags')
        <title>{{ shopName() . ' :: ' . __('general.meta_new') }}</title>
        <meta name="description" content="{{ __('general.meta_new_description') }}">
    @endsection
    ...
    <div class="pt-4 md:pt-6">
        ...
        <div class="max-w-screen-2xl mx-auto px-2 md:px-8 mb-1 md:mb-2">
            <nav class="flex items-center gap-2 text-sm font-medium">
                ...
                <span class="font-black text-black uppercase">{{ __('general.new_products') }}</span>
            </nav>
        </div>

        <!-- Page Title -->
        <div class="max-w-screen-2xl mx-auto px-4 md:px-8 pb-8">
            <h1 class="text-4xl md:text-6xl font-black text-black mb-2">{{ __('general.new_products') }}</h1>
            <p class="text-lg font-medium hidden md:block">{{ __('general.found_new_products', ['count' => $products->total()]) }}</p>
        </div>
        ...
        <div class="max-w-screen-2xl mx-auto px-4 md:px-8">
            ...
            <div class="mb-6 md:mb-8">
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4 md:mb-6">
                    ...
                    <div class="relative">
                        <button id="filter-btn" 
                                onclick="document.getElementById('filter-modal').classList.add('active'); document.body.style.overflow = 'hidden';"
                ...
                @if($selected_filters || $selected_categories)
                    ...
                    <div class="border-4 border-green-600 bg-green-50 p-3 md:p-4 mb-4 md:mb-6">
                                ...
                                @foreach($filter_group as $filter)
                                    @if(in_array($filter->filter_id, $selected_filters))
                                        <button wire:click="removeFilter({{ $filter->filter_id }})" 
                                                ...
                                                class="bg-green-600 text-white px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold hover:bg-green-700 transition-colors">
                                            ✕ {{ $filter->filter_title }}
                                        </button>
                                    @endif
                                @endforeach
                            @endforeach
                            <button wire:click="clearFilters" class="bg-green-600 text-white px-2 sm:px-3 py-1 text-xs sm:text-sm font-bold hover:bg-green-700 transition-colors">
            ...
            <div id="filter-modal" class="filter-modal-overlay">
                <div class="filter-modal-content">
                    ...
                    <div class="bg-black text-white p-6 flex items-center justify-between">
                        <h2 class="font-black text-xl lg:text-2xl">{{ __('general.filter_new') }}</h2>
                        <button id="close-filter-modal" 
                                onclick="document.getElementById('filter-modal').classList.remove('active'); document.body.style.overflow = 'auto';"
            ...
            @else
                ...
                <div class="border-4 border-black bg-white p-8 md:p-16 text-center mb-12">
                    ...
                    <h2 class="text-2xl md:text-4xl font-black mb-4">{{ __('general.no_new_products') }}</h2>
                    <p class="text-lg font-medium mb-8">{{ __('general.no_new_products_message') }}</p>
                    <button wire:click="clearFilters"
